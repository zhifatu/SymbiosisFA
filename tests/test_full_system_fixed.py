...
def test_philosophical_principles_fixed():
    """验证哲学原则 - 修复导入问题"""
    print("\n" + "=" * 50)
    print("测试4：哲学原则验证")
    print("=" * 50)
    ...
    try:
        ...
        alive_person = Individual(
            id="alive_test",
            life_state=LifeState.ALIVE,
            primal_strength=PrimalValue(0.1)
        )
        extinct_person = Individual(
            id="extinct_test",
            life_state=LifeState.EXTINCT,
            primal_strength=PrimalValue(0.8)
        )

        env = Environment()  # 现在正确导入了
        ...
        if kunzhuan:
            ...
            principle3_passed = True
        else:
            print(f"   场维持正常，无需坤转")
            principle3_passed = False
...
def main():
    ...
    tests = [
        ...
        ("哲学原则", test_philosophical_principles_fixed),  # 使用修复版
